...
public class OrderEntryController {
    ...
    public void initialize() {
        ...
        orderItemsObservableList = FXCollections.observableArrayList();
        orderTable.setItems(orderItemsObservableList);
        ...
        paymentCombo.getSelectionModel().selectedItemProperty().addListener(this::onPaymentChanged);
        cashReceivedField.textProperty().addListener((obs, oldVal, newVal) -> updateChange());

        filterSizeCombo.getSelectionModel().selectFirst();
        filterPriceOperatorCombo.getSelectionModel().selectFirst();
    ...
    private void applyFilters() {
        ...
        if (currentCategoryFilter != null) {
            products = inventoryService.findProductByCategoryFilter(products, currentCategoryFilter);
        }
        ...
        String sizeSelection = filterSizeCombo.getSelectionModel().getSelectedItem();
        ...
        if (priceOp != null && !priceOp.equals("Qualquer preço") && priceText != null && !priceText.isBlank()) {
            try {
                double price = Double.parseDouble(priceText.replace(",", ".").replace("R$", "").trim());
    ...
    private void addProductToOrder(Product product) {
        ...
        if (alreadyInCart + 1 > product.getStockQtd()) {
    ...
    private void finalizeOrder() {
        ...
        if (paymentLabel == null || paymentLabel.isBlank()) {
        ...
        if (method == PaymentMethods.CASH) {
            try {
                cashReceived = parseMoney(cashReceivedField.getText());
            } catch (NumberFormatException e) {
                showError("Valor inválido", "Informe um valor recebido válido.");
                return;
            }
        }

        currentOrder.setCustomerName(customerNameField.getText());

        try {
            checkoutService.processPayment(method, currentOrder, cashReceived);
            checkoutService.finishSale(currentOrder, method, cashReceived);

            double change = method == PaymentMethods.CASH
                    ? checkoutService.calculateChange(currentOrder.getTotalCost(), cashReceived)
                    : 0;

            String receipt = receiptService.buildReceipt(
